...
public class movement_2 : MonoBehaviour
{
    ...
    {
        if (Physics2D.Raycast(transform.position, Vector2.right, halfWidth + 0.1f, LayerMask.GetMask("Area")) && rb.linearVelocity.x > 0)
        {
            currD *= -1;

        }
        else if (Physics2D.Raycast(transform.position, Vector2.left, halfWidth + 0.1f, LayerMask.GetMask("Area")) && rb.linearVelocity.x < 0)
        {
            currD *= -1;
